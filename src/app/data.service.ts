...
export class DataService {
  ...
  getCategories(): Observable<string[]> {
    const categories = this.artisans.map((artisan) => artisan.category);
    return of([...new Set(categories)]);
  }

  getTopArtisans(): Observable<any[]> {
    const topArtisans = this.artisans.filter((artisan) => artisan.top);
  ...
  getArtisans(): Observable<any[]> {
    // Retourner directement les données des artisans stockées localement
